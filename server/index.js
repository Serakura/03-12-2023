...
const BevStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    // Folder file uploadnya gabisa dibuat di folder public fe karena folder publicnya fe gabisa diakses dari server
    // ntar tinggal bersih2in aja multer storage yg udah ga kepake
    // takutnya nanti datanya double
    cb(null, 'assets'); // Set the destination folder where uploaded files will be stored
  },
  filename: function (req, file, cb) {
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    const fileExtension = file.originalname.split('.').pop();
